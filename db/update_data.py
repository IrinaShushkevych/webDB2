...
def update_grade(id_student, id_subject, date, new_grade):
    grade = session.query(Gradebook).filter(and_(Gradebook.id_student == id_student, Gradebook.id_subject == id_subject, Gradebook.createdAt == date)).one()
    if not grade:
