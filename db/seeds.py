...
def fill_db():
    ...
    students_list = [Students(student_name=el[0], group=groups_list[el[1] - 1]) for el in data['students']]
    teachers_list = [Teachers(teacher_name=el[0]) for el in data['teachers']]
    subjects_list = [Subjects(subject_name=el[0], teacher=teachers_list[el[1] - 1]) for el in data['subjects']]
    grades_lists = [Gradebook(
